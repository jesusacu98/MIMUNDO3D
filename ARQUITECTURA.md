...
| `/admin/disenos` | Generador de diseños | Descripción + imágenes de referencia opcionales (arrastrar y soltar, varias a la vez) → 1-4 bocetos/mockups de OpenAI (modelo de imagen elegible por generación), con reglas de imprimibilidad FDM en el prompt. Cada boceto tiene «Generar STL» (código OpenSCAD escrito por IA y renderizado en WASM, ver §13). Reusa la llave de `/admin/ideas/configuracion` — ver §12. |
...
- (`schema_catalog_v8.sql`) `slug` (único; enlace `/categorias/<slug>`), `description`, `headline` (título llamativo opcional) `show_on_home` (bool: sale en «Explora por categoría» del inicio) y `home_image_url` (imagen de su recuadro, subida desde `/admin/categorias` al bucket `imagenes_sitio`, carpeta `categorias/`; si falta, el inicio usa los banners `home_category`). Se editan en `/admin/categorias`; `lib/siteCategories.ts` los lee y, si la consulta falla (migración sin correr), cae a la lista fija de `lib/homeCategories.ts`. La cabecera y el pie siguen usando esa lista fija.
...
### Del boceto al STL («Generar STL», 2026-09-24)

1. `DisenosForm.tsx` reduce el boceto a JPEG (canvas) y hace `POST /api/admin/disenos/stl` con `{ image, description, maxSizeMm }`. La ruta re-verifica el rol admin y usa la llave y el modelo de texto de `idea_settings` (`getIdeaSettings()`), sin configuración propia.
2. `lib/disenos/cad.ts` manda el boceto (visión) + descripción + tamaño máximo a `client.responses.create()` con un prompt de sistema que pide código OpenSCAD imprimible (sólido único, base plana en z=0, paredes ≥1.2mm, sin `text()`/`minkowski()`/`include`).
3. `lib/disenos/scad.ts` valida el código (rechaza construcciones prohibidas o `$fn` > 128) y lo renderiza con `openscad-wasm-prebuilt`. El WASM es el sandbox: el código generado por la IA nunca corre como JS de Node. Convierte el STL ASCII a binario (5x menos peso) y calcula las medidas.
4. Si el render falla o OpenSCAD avisa (módulo inexistente, malla no hermética), se hace **una** segunda pasada devolviéndole el aviso al modelo. Si ese intento también falla pero el primero salió, se devuelve el primero con sus avisos.
5. Respuesta: STL binario en base64, el código `.scad` (editable a mano), medidas y avisos. Sin persistencia, igual que las imágenes.

Límites conocidos: el WASM corre en el mismo hilo y no se puede cortar por timeout (por eso se prohíben de entrada las construcciones lentas; `maxDuration = 120`); no hay fuentes, así que el texto no se modela; las formas orgánicas complejas salen simplificadas. Los fallos de diseño (`ScadError`) no disparan el correo de aviso; sólo los errores del proveedor (`OpenAI.APIError`).
