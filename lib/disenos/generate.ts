...
export interface GenerateDesignSketchesInput {
  ...
  model: string;
  prompt: string;
  /** Logo de referencia, si el usuario adjuntó uno. */
  referenceLogo?: File | null;
  count: number;
}

/** Genera bocetos/mockups (no un modelo 3D) en base64, listos para mostrar/descargar. */
export async function generateDesignSketches({ apiKey, model, prompt, referenceLogo, count }: GenerateDesignSketchesInput): Promise<string[]> {
  ...
  const response: OpenAI.ImagesResponse =
    referenceLogo && referenceLogo.size > 0
      ? await client.images.edit({ model, image: referenceLogo, prompt, n, size: '1024x1024', quality: 'auto' })
